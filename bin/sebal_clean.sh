...
function sebalCleanup {
	...
	ssh $SSH_OPTIONS -i $private_key_path azureuser@$scheduler_ip "$COMMAND_TRUNCATE_NASA_IMAGES"
	...
	ssh $SSH_OPTIONS -i $private_key_path azureuser@$scheduler_ip "$COMMAND_TRUNCATE_STATES_TIMESTAMPS"
}