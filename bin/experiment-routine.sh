...
# Scripts Constants
SEBAL_AUTOMATION_CONTROL=bin/control-fix.sh
...
do
  ...
  if [ $used_disk -lt 79 ]
  then
    bash $SEBAL_AUTOMATION_CONTROL crawler-allocate --site $SITE_LSD --file $LSD_FAKE_FILE_NAME"_"$count --size $LSD_FAKE_FILE_SIZE
    count=$((count + 1))
  else
    remains_disk=false
  fi
